...
随着倾斜技术的日趋成熟，超图三维研发团队形成了一套C/S下成熟的倾斜数据的解决方案，接下来，如何在Web端应用倾斜数据成为一个迫切的需求。


OSGB格式，被认为是倾斜摄影模型的文件格式标准，其数据规范很好的解决了海量三维模型数据的高效浏览，但数据结构过于复杂，性能消耗和解析时间都无法满足Web的用户体验。为了改进海量异构数据集的传输和渲染性能， Cesium推出了开发标准3D-Tiles，一种空间数据结构、一套为三维而生的切片格式。
