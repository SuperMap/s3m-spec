...
struct BatchSelAndVertexColor
{
	...
	std::map<uInt, std::vector<unsigned short>> mapInstanceIdVec;
	//实例ID与原始的顶点颜色的map
	std::map<uInt, uInt> mapInstanceColor;
};
...
public:
	...
	//用4位存储RGBA
	static uInt RGBA(Byte r, Byte g, Byte b, Byte a);
	//将一个数值限制到一定范围内. 
	template <typename T>
	...
	//解压S3MB文件流并释放压缩流内存
	static bool UnZipData(MemoryStream &streamUnZipped, Byte*& pZippedData, uInt nZippedSize);
	...
	//加载纹理坐标信息
	static bool LoadTextureCoords(MemoryStream &streamSkeleton, VertexDataPackage*& pVertexDataPackage, USHORT nTextureCoord);
	//加载实例化信息
	static bool LoadInstanceInfo(MemoryStream &streamSkeleton, VertexDataPackage*& pVertexDataPackage, USHORT nTextureCoord);
	//判断是否是实例化几何对象
	...
	//解压被压缩的法线信息
	static bool DecompressNormal(uInt nNormalCount, short* pEncodeNormal, float*& pNormal);
	//加载压缩纹理坐标信息
	static bool LoadCompressTextureCoords(MemoryStream &streamSkeleton, VertexDataPackage*& pVertexDataPackage, USHORT nTextureCoord);
	//处理单个压缩纹理坐标信息
	static bool LoadSingleCompressTextureCoords(MemoryStream &stream, uInt nTexIdx, uInt nTotalTexCount, VertexDataPackage* pVertexDataPackage);
	...
	static void RangeEncode(float* pValue, uInt nVertexCount, uInt nSrcDimension, uInt nDstDimension, uInt nQuantizationBits, short*& pEncodeValue,
	...
	static void ComputeMaxMin(float* pValue, uInt nDimension, uInt nVertexCount, float* pOutMin, float* pOutMax);
	//! \brief 保存压缩的法线
	static bool SaveCompressNormal(VertexDataPackage* pVertexDataPack, MemoryStream &stream);
	//! \brief 压缩法线
	static bool CompressNormal(uInt nNormalCount, float* pNormal, short*& pEncodeNormal);
	//! \brief 保存压缩的纹理坐标
	static bool SaveCompressTextureCoords(VertexDataPackage* pVertexDataPack, MemoryStream &stream);
	//! \brief 保存压缩的一重纹理坐标
	static bool SaveOneCompressTextureCoords(VertexDataPackage* pVertexDataPack, MemoryStream &stream, uInt nTexIdx);
	...
	static void SaveTextureData(TextureData* pTextureData, \
		MemoryStream &streamTexture, uInt nCompressType, bool bIsChangeTexture = TRUE);
	//! \brief 纹理压缩处理
	static void ProcessTextureData(TextureData* pTextureData, uInt nCompressType) { return ProcessTextureData(pTextureData, (CodecType)nCompressType); }
	...
	static bool ZipData(byte *pvDestBuffer, uInt &dwDestLen,
		const byte *pvSrcBuffer, uInt dwSrcLen, int nLevel = 8);
};
