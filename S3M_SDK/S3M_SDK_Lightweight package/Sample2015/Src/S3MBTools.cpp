...
unsigned int S3MBTools::RGBA(unsigned char r, unsigned char g, unsigned char b, unsigned char a)
{
	unsigned int colorValue = 0;
	if (ISBIGENDIAN)
	{
		colorValue = (((unsigned int)(unsigned char)(r) << 24) | ((unsigned int)(unsigned char)(g) << 16) | ((unsigned int)(unsigned char)(b) << 8) | ((unsigned int)(unsigned char)(a)));
	}
	else
	{
		colorValue = (((unsigned int)(unsigned char)(r)) | ((unsigned int)(unsigned char)(g) << 8) | ((unsigned int)(unsigned char)(b) << 16) | ((unsigned int)(unsigned char)(a) << 24));
	}
	return colorValue;
}
	...
	MemoryStream& streamSecondColor, MemoryStream& streamTexture, MemoryStream& streamSelInfo, string& strJsonMaterials)
{
	...
	fileStream.read((char*)&nZippedSize, sizeof(unsigned int));
	...
	//fileStream.read((char*)&nOptions,sizeof(unsigned int));
...
bool S3MBTools::UnZipData(MemoryStream& streamUnZipped, unsigned char*& pZippedData, unsigned int nZippedSize)
{
	...
	while (!bResult)
	{
		...
		if (!bResult)//如果解压失败.
		{
			...
			pUnZipData = new(std::nothrow) unsigned char[nUnZipSize];
			continue;
		}
	}
	delete pZippedData;
...
{
	unsigned int nTagValue = SV_Unkown;
...
bool S3MBTools::LoadSecondVertexColor(MemoryStream &streamSkeleton, VertexDataPackage*& pVertexDataPackage)
{
	...
	pVertexDataPackage->m_pSecondVertexColor = new(std::nothrow) unsigned int[pVertexDataPackage->m_nSecondVertexColorCount];
	for (unsigned int j = 0; j < pVertexDataPackage->m_nSecondVertexColorCount; j++)
	{
		unsigned char btR, btG, btB, btA;
...
bool S3MBTools::LoadInstanceInfo(MemoryStream &streamSkeleton, VertexDataPackage*& pVertexDataPackage, unsigned short nTextureCoord)
{
	unsigned char nByte = 0;
	...
	for (int iIndex = 0; iIndex < nInstanceInfo; iIndex++)
	{
		...
		if (pVertexDataPackage->m_nTexDimensions[i] == SIZE_PER_INSTANCE)
		{
			...
			unsigned int* pUint = (unsigned int*)pValue;
			for (unsigned int kIndex = 0; kIndex < pVertexDataPackage->m_TexCoordCount[i]; kIndex++)
			{
...
bool S3MBTools::LoadCompressVertex(MemoryStream &streamSkeleton, VertexDataPackage*& pVertexDataPackage)
{
	...
	unsigned short* pCompressVertices = new(std::nothrow) unsigned short[pVertexDataPackage->m_nVerticesCount * pVertexDataPackage->m_nVertexDimension];
...
bool S3MBTools::DecompressNormal(unsigned int nNormalCount, short* pEncodeNormal, float*& pNormal)
{
...
bool S3MBTools::LoadCompressTextureCoords(MemoryStream &streamSkeleton, VertexDataPackage*& pVertexDataPackage, unsigned short nTextureCoord)
{
	unsigned char nByte = 0;
	streamSkeleton >> nTextureCoord;
	streamSkeleton >> nByte;
...
bool S3MBTools::LoadSingleCompressTextureCoords(MemoryStream &streamSkeleton, unsigned int nTexIdx, unsigned int nTotalTexCount, VertexDataPackage* pVertexDataPackage)
{
	unsigned char nByte = 0;
	unsigned int nTexCoordCount = 0;
	bool bNeedTexCoordZ;
	...
	unsigned short compressDimension = 0;
...
void S3MBTools::LoadTextureData(MemoryStream &streamTexture, TextureData* pTextureData)
{
	...
	streamTexture >> (unsigned char&)pTextureData->m_enFormat;
...
void S3MBTools::ProcessTextureData(TextureData* pTextureData, CodecType nCompressType)
{
	...
	else//encNONE
	{
		if (comp == 3)//RGB->RGBA
		{
			...
			for (unsigned int y = 0; y < (unsigned int)img.nHeight; y++)
			{
				...
				{
					unsigned int nValue = ImgToolkit::GetValue((unsigned char*)img.pBits, nImgWidthBytes, (PixelFormat)img.btBitsPixel, x, y);
	...
	if (pTextureData->m_enFormat == PF_BYTE_LA)
	{
		...
		pDecodeOut = new unsigned char[nDecodeSize];
		for (unsigned int y = 0; y < (unsigned int)pTextureData->m_nHeight; y++)
		{
			for (unsigned int x = 0; x < (unsigned int)pTextureData->m_nWidth; x++)
			{
				...
				unsigned char alpha = pTextureData->m_pBuffer[2 * nTexIndex];
				unsigned char color = pTextureData->m_pBuffer[2 * nTexIndex + 1];
...
bool S3MBTools::SaveSelectionInfo(std::map<string, Geometry*>& m_mapGeometry, MemoryStream &m_streamSelectionInfo)
{
	...
	for (; it != m_mapGeometry.end(); it++)
	{
		...
		{
			std::map<unsigned int, std::vector<SelectInfo*> > dictSelectInfo;

			unsigned int nID = pVertexDataPackage->m_pSecondVertexColor[0];
			...
			{
				std::map<unsigned int, std::vector<SelectInfo*> >::iterator itorCurrentSel;
				unsigned int nCurrentID = pVertexDataPackage->m_pSecondVertexColor[j];
			...
			std::map<unsigned int, std::vector<SelectInfo*> >::iterator posSelectInfo;
			unsigned int nDictID = 0;
			std::vector<SelectInfo*> arrInfo;

			m_streamSelectionInfo << pGeometry->m_strGeoName;

			unsigned int nCurrentID = 0xffffffff;
			int nRemove = 0;
			std::map<unsigned int, std::vector<SelectInfo*> >::iterator itorCurrentSel;
			itorCurrentSel = dictSelectInfo.find(nCurrentID);
			if (itorCurrentSel != dictSelectInfo.end())
		...
		else
		{
			...
			std::map<unsigned int, BatchSelAndVertexColor* > dictSelectInfo;
			...
			std::map<unsigned int, BatchSelAndVertexColor* >::iterator posSelectInfo;
			unsigned int nDictID = 0;
			...
			for (posSelectInfo = dictSelectInfo.begin(); posSelectInfo != dictSelectInfo.end(); ++posSelectInfo)
			{
				...
				for (int j = 0; j < nSize; j++)
				{
					unsigned int nInstanceIndex = arrInfo->mapInstanceIdVec[nDictID][j];
...
bool S3MBTools::ZipData(unsigned char *pvDestBuffer, unsigned int &dwDestLen, const unsigned char *pvSrcBuffer, unsigned int dwSrcLen, int nLevel /*= 8*/)
{
	unsigned long ulDestLen = dwDestLen;	//	WhiteBox_Ignore
	// 这里搞得这么麻烦，是因为compress2的第二个参数是一个指向unsigned long型的指针，需要类型完全匹配，
	// unsigned int实际上就是unsigned int，而unsigned long在64位系统（如Solaris）上是64位的，就出问题了
...
bool S3MBTools::SaveSecondVertexColor(VertexDataPackage* pVertexDataPack, MemoryStream &stream)
{
	...
	for (unsigned int j = 0; j < pVertexDataPack->m_nSecondVertexColorCount; j++)
	{
		unsigned int nColor = pVertexDataPack->m_pSecondVertexColor[j];
		unsigned char btR = Palette::REDVAL(nColor);
		unsigned char btG = Palette::GREENVAL(nColor);
		unsigned char btB = Palette::BLUEVAL(nColor);
		unsigned char btA = Palette::ALPHAVAL(nColor);
...
bool S3MBTools::SaveInstanceInfo(VertexDataPackage* pVertexDataPack, MemoryStream &stream)
{
	...
	for (int i = 0; i < SMSCN_MAX_TEXTURE_COORD_SETS; i++)
	{
		...
		if (pVertexDataPack->m_nTexDimensions[i] == SIZE_PER_INSTANCE)
		{
			...
			unsigned int* pUint = (unsigned int*)pValue;
			for (unsigned int kIndex = 0; kIndex < pVertexDataPack->m_TexCoordCount[i]; kIndex++)
			{
				// 三个纹理坐标存储矩阵，一个纹理坐标存储选择ID，一个颜色存储选中时顶点颜色
				stream.Save(pValue + kIndex * SIZE_PER_INSTANCE, 16);

				// secondColor拆分为4个unsigned char存储，方便web端使用
				unsigned int nColor = pUint[SIZE_PER_INSTANCE * kIndex + 16];
				unsigned char btR = Palette::REDVAL(nColor);
				unsigned char btG = Palette::GREENVAL(nColor);
				unsigned char btB = Palette::BLUEVAL(nColor);
				unsigned char btA = Palette::ALPHAVAL(nColor);
		...
		else if (pVertexDataPack->m_nTexDimensions[i] == SIZE_PER_DM_INSTANCE)
		{
			...
			for (unsigned int kIndex = 0; kIndex < pVertexDataPack->m_TexCoordCount[i]; kIndex++)
			{
				...
				// vertexColor和secondColor拆分为4个unsigned char存储，方便web端使用
				unsigned int nColor = pUint[SIZE_PER_DM_INSTANCE * kIndex + 27];
				unsigned char btR = Palette::REDVAL(nColor);
				unsigned char btG = Palette::GREENVAL(nColor);
				unsigned char btB = Palette::BLUEVAL(nColor);
				unsigned char btA = Palette::ALPHAVAL(nColor);
...
void S3MBTools::RangeEncode(float* pValue, unsigned int nVertexCount, unsigned int nSrcDimension, unsigned int nDstDimension, unsigned int nQuantizationBits, short*& pEncodeValue, float& fNormalConstant, float*& minVal)
{
	float maxVal[4];
	float rangVal[4];
	ComputeMaxMin(pValue, nSrcDimension, nVertexCount, minVal, maxVal);
	for (unsigned int i = 0; i < nDstDimension; i++)
	{
	...
	{
		for (unsigned int j = 0; j < nDstDimension; j++)
		{
...
void S3MBTools::ComputeMaxMin(float* pValue, unsigned int nDimension, unsigned int nVertexCount, float* pOutMin, float* pOutMax)
{
	for (int i = 0; i < 4; i++)
	{
		pOutMin[i] = 0.0;
...
bool S3MBTools::CompressNormal(unsigned int nNormalCount, float* pNormal, short*& pEncodeNormal)
{
...
bool S3MBTools::SaveOneCompressTextureCoords(VertexDataPackage* pVertexDataPack, MemoryStream &stream, unsigned int nTexIdx)
{
	unsigned char nByte = 0;
	...
	unsigned int nTexCoordCount = pVertexDataPack->m_TexCoordCount[nTexIdx];
	...
	{
		float* pTexCoordZ = new(std::nothrow) float[nTexCoordCount];
		for (unsigned int i = 0; i < nTexCoordCount; i++)
		{
...
void S3MBTools::SaveTextureData(TextureData* pTextureData, MemoryStream &streamTexture, unsigned int nCompressType, bool bIsChangeTexture /*= true*/)
{
	if (bIsChangeTexture)
	{
