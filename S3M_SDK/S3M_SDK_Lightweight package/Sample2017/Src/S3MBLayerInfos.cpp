...
bool S3MBLayerInfos::LoadAttributeDataFromFile(const string& strAttFilePath, std::map<unsigned int, Feature*>& mapFeature)
{
	//读取文件并解压
...
bool S3MBLayerInfos::GetFeatureValuesFromJson(Feature *pFeature, JsonValue & jsonValue)
{
	...
	for (int i = 0; i < nFeatureSize; i++)
	{
		...
		case S3mbFieldInfo::Char:
		{
			...
			pByte = new unsigned char[strValue.size()+1];
		...
		case S3mbFieldInfo::LongBinary:
		{
			...
			pByte = new unsigned char[nSize * sizeof(double)];
