...
public:
	...
	//! \brief 材质是否存在
	bool IsExistToMaterial(string strMaterialName);

	//! \brief 纹理是否存在
	bool IsExistToTexData(string strTextureName);

	//! \brief Node是否存在
	bool IsExistToGeometry(string strName);

	//! \brief 添加一个材质
	//! \param strMaterialName 材质名，可以和Material3D里的名字不对应[in]
	//! \param pMaterial3D 材质信息[in]
	//! \return 重复材质返回false[out]
	bool AddMaterial(string strMaterialName, Material* pMaterial);

	//! \brief 添加一个纹理信息
	//! \param strTextureName 纹理名[in]
	//! \param pTexData 纹理信息[in]
	//! \return 重复纹理返回false[out]
	bool AddTextureData(string strTextureName, TextureDataInfo* pTexData);

	//! \brief 添加一个Geometry信息
	//! \param strName Geometry名[in]
	//! \param pGeometry Geometry信息[in]
	//! \return 重复返回false[out]
	bool AddGeometry(string strName, Geometry* pGeometry);

	//! \brief 获得材质
	Material* GetMaterial(string strMaterialName);
	//! \brief 获得材质
	std::map<string, Material*>& GetMaterials();

	//! \brief 获得纹理
	TextureDataInfo* GetTextureData(string strTextureName);
	//! \brief 获得所有纹理
	std::map<string, TextureDataInfo*>& GetTextureData();

	//! \brief 获得Geometry
	Geometry* GetGeometry(string strName);

	//! \brief 获得Geometry
	std::map<string, Geometry*>& GetGeometry();
	...
	//! \brief 移除指定Geometry
	virtual void RemoveGeometry(string strName);
...
protected:
	...
	//! \brief 纹理信息
	std::map<string, TextureDataInfo*> m_mapTextureData;

	//! \brief Node信息
	std::map<string, Geometry*> m_mapGeometry;
};

class RenderOperationGroup : public RenderOperationNode
{
public:
	...
	//! \brief 类名
	virtual string ClassName() { return "Group"; }
...
class RenderOperationPagedLOD : public RenderOperationGroup
{
public:
	...
	//! \brief 类名
	virtual string ClassName() { return "PagedLOD"; }

	//! \brief 获得范围
	std::vector<std::pair<float, float> >& GetRanges(){ return m_vecRangeList; }

	//! \brief 获得子切片
	std::vector<string>& GetFileNames(){ return m_vecFileName; }

	//! \brief 获取子切片的包围球
	std::map<string, BoundingSphere>& GetLODSphere(){ return m_mapBoundingSphere; 
}
...
private:
	...
	//! \brief LOD切片名
	std::vector<string> m_vecFileName;

	//! \brief LOD切片包围盒
	std::map<string, BoundingSphere> m_mapBoundingSphere;
...
class  RenderOperationGeode : public RenderOperationNode
{
public:
	...
	//! \brief 类名
	virtual string ClassName() { return "Geode"; }
